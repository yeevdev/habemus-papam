...
public class TimeUI : MonoBehaviour
{
    ...
    public void Reset()
    {
        ...
        LeftText.text = $"Day {currentDay}\n{currentCon}";
        RightText.text = $"남은 시간\n{(int)(InGameManager.Instance.Balance.MaxConclaveTime/60)} : {(InGameManager.Instance.Balance.MaxConclaveTime%60).ToString("00.00")}";
    ...
    {
        LeftText.text = $"Day {InGameManager.Instance.GetCurrentDay()}\n{InGameManager.Instance.GetCurrentConclave()}";
        RightText.text = $"남은 시간\n{(int)(InGameManager.Instance.Balance.MaxConclaveTime/60)} : {(InGameManager.Instance.Balance.MaxConclaveTime%60).ToString("00.00")}";
        ClockHand.transform.rotation = Quaternion.identity;
    ...
    void Update()
    {
        ...
        RightText.text = $"남은 시간\n{(int)(remain/60)} : {(remain%60).ToString("00.00")}";
    }
}